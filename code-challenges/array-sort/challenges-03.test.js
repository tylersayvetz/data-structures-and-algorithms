...
//TODO: nice! When I looked up the UTF-8 charts, I found that all capital letters have lower UTF8 codes than lower case. What if you wanted to do this the other way around???
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const sortMeetingsByDay = (arr) => {
  ...
  //new array contains objects of weekdays and indexes
  const days = [
    { day: 'Monday', index: 1 },
    { day: 'Tuesday', index: 2 },
  ...
  //iterate through arr and days, match index to arr.dayOfWeek and add new index property to arr
  arr.forEach(meeting => {
    days.forEach(day => {
      if (day.day === meeting.dayOfWeek) {
        meeting.dayNum = day.index;
      }
    })
  })

  //sort by dayNum
  arr.sort((a, b) => a.dayNum < b.dayNum ? -1 : 1);

  //delete dayNum property.
  arr.forEach(meeting => {
    delete meeting.dayNum;
  })
...
TODO: TYLER: this instruction doesn't handle the case where: two meetings start at the same time on different days. What should we do?. 
Tyler: Also, a fring case would be if a meeting spills over past midnight. That would break my solution... 
...
const sortSchedule = (arr) => {
  ...
  arr.forEach(meeting => {
    days.forEach(day => {
      console.log(day);
      if (meeting.dayOfWeek === day) {
        meeting.idx = days.indexOf(day);
      }
    })
  })
