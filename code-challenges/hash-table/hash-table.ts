...
*/
const SECRET_PRIME: number = 599;

// interface HashTable {
//   length: number
//   storage: Array<HashNode> | Array<null>
// }
...
class HashNode {
  constructor(item: Data, next: Data | null) {
    this.item = item;
    this.next = null;
  }
}
...
class HashTable {
  ...
  add(item: Data) {
    ...
    if (this.storage[index] === null) this.storage[index] = newNode
    else {
      let depth = 0
  ...
  hash(key: string): number {
    let hash: number = key.charCodeAt(0);
