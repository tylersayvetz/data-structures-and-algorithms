...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(val => {
    return !forbiddenValues.find(forb => forb === val);
  })
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(statt => statt.baseStat > minBaseStat).map(stat => stat.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(person => !person.children).map(person => {
    return {
      house: person.house,
      name: person.name,
...
const evenOddNumericValues = (arr) => {
  return arr.filter(el => typeof el === 'number').map(num => num % 2 === 0 ? 'even' : 'odd');
};
